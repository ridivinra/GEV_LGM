...
#### Get the data ready
# Read in the data
load("catchDesc.RData")
load("flowData.RData")
load("flowDataSample.RData")
# Select only stations with 10 or more observations
stations_10 <- flowData %>% group_by(Station) %>% summarise(n = n()) %>% filter(n>=10)
# Use only stations suitable for pooling and QMED
all_good <- catchDesc %>% 
  filter(suitQMED == "YES", suitPool == "YES", 
         catchDesc$FPEXT != -9.9990, Station %in% stations_10$Station)
# Remove stations from flow data
dataF <- flowData %>% filter(Station %in% all_good$Station)
stations <- unique(dataF$Station)
...
for(i in 1:n_st){
  station <- stations[i]
  currentS <- dataF %>% filter(Station == station)
  ...
  GEV_fit <- nlm(fn, theta <- theta0, y <- currentS$Flow, t <- currentS$WaterYear, hessian = T)

  S_d <- try(solve(GEV_fit$hessian),silent=T)
  if(class(S_d_a) == "matrix"){
    psi_sd <- sqrt(S_d_a[1,1])
    tau_sd <- sqrt(S_d_a[2,2])
    kappa_sd <- sqrt(S_d_a[3,3])
    gamma_sd_a <- sqrt(S_d_a[4,4])
  }else{
    gamma_sd_a = NA
  }
...
covariates <- matrix(c(
  rep(1,n_st),
  transform(data$Area, "AREA"),
  transform(data$SAAR, "SAAR"),
  transform(data$FARL, "FARL"),
  transform(data$BFIHOST, "BFIHOST"),
  transform(data$FPEXT, "FPEXT"),
  transform(data$URBEXT2000, "URBEXT"),
  transform(data$DPLBAR, "DPLBAR"),
  transform(data$DPSBAR, "DPSBAR"),
  transform(data$LDP, "LDP"),
  transform(data$SPRHOST, "SPRHOST"),
  transform(data$ASPBAR, "ASPBAR"),
  transform(data$ALTBAR, "ALTBAR"),
  transform(data$ASPVAR, "ASPVAR"),
  transform(data$PROPWET, "PROPWET")
), ncol = 15, nrow = n_st
)
colnames(covariates) <- names

# add coords to MLE data
# Hér var ég
dt_mles <- dt_mles %>% 
  left_join(all_good %>% dplyr::select(Station, Ceasting, Cnorthing), c("Station" = "Station")) %>% 
  rename(Easting = Ceasting, Northing = Cnorthing)
...
coords_obj <- convertCoords(dt_mles)
...
################# Ná í mat á variance og mode fyrir hyper stika fyrir proposal distribution ###############################
...
Sigma_kappa_0 <- diag(c(tau_psi_0$sd^2, rho_psi_0$sd^2, sigma_psi_0$sd^2,
                        ...
                        tau_xi_0$sd^2, tau_gamma_0$sd^2)
                      )*(2.38^2/8)
loops <- 100
burnIn <- 0.2*loops
...
sample_mcmc <- function(x, kappa_0, Sigma_kappa_0, loops){
  #set.seed(x)
  ...
  N_loops <- loops
  kappa_mat <- matrix(NA, nrow = length(kappa_k), ncol = N_loops)
  ...
  write.table(x = accept, file = paste0("acceptNew",x,".txt"))
  write.table(x = kappa_mat, file = paste0("chainNew",x,".csv"))
}
start <- proc.time()
save_fit <- mclapply(c(1,2,3,4), sample_mcmc, kappa_0 = kappa_0, Sigma_kappa_0 = Sigma_kappa_0, loops = loops)
stop <- proc.time()- start


firstCh <- read.table("chainNew1.csv") %>% as.matrix()
secondCh <- read.table("chainNew2.csv") %>% as.matrix()
thirdCh <- read.table("chainNew3.csv") %>% as.matrix()
forthCh <- read.table("chainNew4.csv") %>% as.matrix()
chain <- cbind(firstCh[,-c(1:burnIn)],secondCh[,-c(1:burnIn)],
               thirdCh[,-c(1:burnIn)],forthCh[,-c(1:burnIn)])
