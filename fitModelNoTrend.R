...
fn <- function(theta,y) {
  ...
  -sum(dgev(y, loc = exp(theta[1]), scale = exp(theta[2] + theta[1]), shape = ((1-(exp(-exp((theta[3] - b3)/sig3))))^(1/alp3) - 0.5), log = TRUE)) - ((alpha - alp3)*log(xitheta + 0.5) + (beta-1)*log(0.5 - xitheta) + (theta[3]-b3)/sig3 - exp((theta[3]-b3)/sig3)  )   
}
...
for(i in 1:n_st){
  ...
  GEV_fit <- nlm(fn, theta <- theta0, y <- currentS$Flow, hessian=TRUE)
...
catchmentDesc <- dt_mles %>% select(Station) %>% left_join(catchmentDesc)
names <- c(
  "Int.",
  "log(Area)",
  "log(SAAR)",
...
# x_mat contains the posterior samples for the latent parameters
...
##############################################  covariate coefficient results ##############################################
# Posterior samples for the covariate coefficients and the spatial components
nu <- x_mat[(3*N+1):nrow(x_mat),]
beta_psi <- nu[1:N_psi,]
u_psi <- nu[1:N_colsA + N_psi,]
beta_tau <- nu[1:N_tau + N_psi + N_colsA,]
u_tau <- nu[1:N_colsA + N_tau + N_psi + N_colsA,]
beta_kappa <- nu[1:N_xi + N_colsA + N_tau + N_psi + N_colsA,]

# results for the covariate coefficients
beta_psi_results <- data.frame(
  Covariate = colnames(X_psi),
  Mean = apply(beta_psi, 1, mean),
  Sd = apply(beta_psi, 1, sd),
  q_025 = apply(beta_psi, 1, quantile, probs = 0.025),
...
beta_tau_results <- data.frame(
  ...
  Mean = apply(beta_tau, 1, mean),
  Sd = apply(beta_tau, 1, sd),
  q_025 = apply(beta_tau, 1, quantile, probs = 0.025),
...
beta_kappa_results <- data.frame(
  ...
  Mean = apply(beta_kappa, 1, mean),
  Sd = apply(beta_kappa, 1, sd),
  q_025 = apply(beta_kappa, 1, quantile, probs = 0.025),
...
# Plot the spatial field on a map of the UK
...
# Get the outlines of Great Britain
map_scaled <- convertCoords(dt = dt_mles)$map_scaled

# Spatial field for psi
